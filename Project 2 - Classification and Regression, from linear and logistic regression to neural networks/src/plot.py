...
X_train = designMatrix_1D(x_train, polygrad)
X_test = designMatrix_1D(x_test, polygrad)


""" Hyperparameters """
n_epochs = 100 #number of epochs
init_LR = 0.5 # Initial learning rate (LR)
decay = 0 #init_LR/n_epochs # LR decay rate (for fixed LR set decay to 0)
momentum = 0 # Momentum value for GD.
minibatch_size = 100
...
""" Results """
y_predict_GD, theta_GD, mse_GD = GD(X_train, X_test, y_train, y_test, G_M, Niterations, init_LR, decay, momentum, seed)
# y_predict_SGD, theta_SGD, mse_SGD = SGD(X_train, X_test, y_train, y_test, O_M, G_M, minibatch_size, n_minibatches, n_epochs, init_LR, decay, momentum, seed)
...
print('\n-------Theta values-------')
print('       ', end='')
print(['theta_' + str(i) for i in range(len(exact_theta))])
print(f'Exact: {exact_theta}')
print(f'GD:    {theta_GD.flatten()}')
# print(f'SGD: {theta_SGD.flatten()}')
print(f'OLS:   {theta_OLS.flatten()}\n')
